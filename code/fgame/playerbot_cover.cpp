...
float BotController::EvaluateCoverQuality(Vector pos, Vector enemyPos)
{
    ...
    trace_t trace = G_Trace(
        eyePos, vec_zero, vec_zero, enemyEyePos, controlledEnt, MASK_SHOT, false, "BotController::EvaluateCoverQuality"
    );
    ...
    if (trace.fraction < BotConstants::TRACE_COMPLETE && trace.entityNum != ENTITYNUM_NONE) {
        ...
        for (int i = 0; i < angleSteps; i++) {
            ...
            trace_t angleTrace = G_Trace(
                ...
                MASK_SHOT,
                false,
                "BotController::EvaluateCoverQuality"
            );
...
bool BotController::IsInCover(Vector pos, Vector enemyPos)
{
    ...
    trace_t trace =
        G_Trace(eyePos, vec_zero, vec_zero, enemyEyePos, controlledEnt, MASK_SHOT, false, "BotController::IsInCover");
...
bool BotController::IsCoverCompromised(void)
{
    ...
    float distanceChange =
        (enemyPos - Vector(coverState.current.position.x, coverState.current.position.y, enemyPos.z)).length();
    ...
    case COVER_NONE:
        ...
        if (level.inttime > m_iLastCoverSearchTime + BotConstants::TARGET_UNSEEN_THRESHOLD) {
            ...
            if (newCover.quality >= g_bot_cover_min_quality->value) {
                ...
                if (g_bot_debug->integer >= 1) {
                    gi.Printf(
                        "[BOT] %s: Moving to cover (quality: %.2f)\n",
                        controlledEnt->client->pers.netname,
                        newCover.quality
                    );
                }
        ...
        if (level.inttime >= coverState.nextPeekTime) {
            ...
            if (g_bot_debug->integer >= 2) {
                gi.Printf(
                    ...
                    controlledEnt->client->pers.netname,
                    coverState.peekDuration / 1000.0f
                );
            }
        }
        break;
